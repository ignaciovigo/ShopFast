    ...
    <section className='h-full w-full flex flex-col gap-5 items-start py-4 px-3'>
      ...
      }
      {
        currentUser.role === 'ADMIN' && (<>
          <InputSearch getProductsBySearch={getProductsBySearch} search={search} setSearch={setSearch} isLoading={isLoading} placeholder='Search by title or category...' />
          <ProductTable getProducts={getProducts} setSearch={setSearch} products={products} isLoading={isLoading} refreshProducts={refreshProducts} />
        </>
          )
        }
      {
      ...
      <div className="flex justify-center items-center gap-3 mt-auto w-full">
        {
            <button  className={`${prevLink ?'visible' :'invisible' } bg-[--bg-200] hover:bg-[--bg-400] text-[--primary-200] hover:text-[--primary-100] font-bold py-2 px-4 rounded active:scale-90 duration-100`} onClick={handleClick} data-btn='prev' ><HiChevronLeft className="pointer-events-none" /> </button> 
        }
        <p className="text-md text-gray-500 cursor-default">{currentPage}</p>
        {
          
            <button  className={`${nextLink ?'visible' :'invisible' } bg-[--bg-200] hover:bg-[--bg-400] text-[--primary-200] hover:text-[--primary-100] font-bold py-2 px-4 rounded active:scale-90 duration-100`} onClick={handleClick} data-btn='next' ><HiChevronRight className="pointer-events-none" /></button>
